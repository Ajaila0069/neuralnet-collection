...
class rnn:
    ...
    def __init__(self, x, names, wpl, lines=[], n_a=50, seq_len=10):
        self.train_x = x
        self.numa = n_a
        ...
        self.lines = [list(self.train_x.tolist()[i]) for i in self.train_x.shape[0]]
        self.tokens = []

        for l in self.lines:
            for element in l:
                sefl.tokens.append(element)
    ...
    def sample(self, seed):
        ...
        indices.append(self.uniquex[len(self.uniquex) - 1])#indices.append(self.ch_index['\n'])
    ...
    def train(self, its=10000, checkpoint=500):
        ...
        for i in range(its):
            ...
            x = [None] + [ch for ch in tokens[index]]#[self.ch_index[ch] for ch in tokens[index]]
            #print(x)
            y = x[1:] + [x[0]]
            #print(y)
            ...
            if (i != 0) and (i % checkpoint == 0):
                ...
                for i in range(self.times):
                    ...
                    stuff = [self.uniquex[i] for i in indices]#[self.index_ch[i] for i in indices]
...
notes = music.turn_to_ints(lines)

whereswaldo = rnn(notes, 8, 10)
print("starting now")
text = whereswaldo.train(iters, 20000)
